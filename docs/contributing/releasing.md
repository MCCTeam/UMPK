...
If a bad package is already public, unlist the affected version on NuGet.org, fix the repository, increment the patch version, and publish a new tag. Do not delete and replace release artifacts under the old version. If NuGet publishing fails before any package is accepted, keep the existing tag unchanged. Rerun the failed job after a transient or account-policy failure. If the workflow itself needs a fix, push the fix to `master`, open the Release workflow in GitHub Actions, choose **Run workflow**, and enter the existing tag.
