...
/// <summary>Data-driven full-matrix live validation (nightly-only, see <see cref="NightlyTheoryAttribute"/>). One representative jar per wire protocol (50 protocols, <see cref="LiveMatrix"/>) is booted sequentially on port 25599 (the <see cref="LiveServerCollection"/> disables parallelism), and each leg drives the high-level <see cref="UmpkClient"/> through login to Play and asserts on real decoded <see cref="ClientState"/>: keep-alive survival, world/terrain decode, chat receive AND send, entity tracking/metadata, a client-driven entity attack, inventory item decode, held-slot selection with a client block place and dig, a chest open/content/click round-trip, potion effects, the command tree, and a clean disconnect - every feature honestly n/a-gated per version by the codec registry. The JVM is version-selected (<see cref="JavaRuntimes"/>) and always torn down; each leg gets one infrastructure-fault reboot-and-retry.</summary>
