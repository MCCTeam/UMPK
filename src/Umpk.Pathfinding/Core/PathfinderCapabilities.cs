...
public sealed record PathfinderCapabilities
{
    ...
    /// <remarks>
    /// <para><b>Why the BOOTS here and the ATTRIBUTE in the engine, and why that asymmetry is deliberate.</b> <see cref="SoulSpeedLevel"/>'s doc argues the boots for <c>movement_efficiency</c> because that attribute is POSITIONAL - it reads 0.0 while the bot stands on stone. That argument does not apply here: <c>water_movement_efficiency</c> is a flat equipment modifier, so the attribute would in fact be honest. The boots are used anyway, for a different reason: <b>the enchantment read works on all 49 protocols and the attribute exists on 16 of them.</b> One code path, one behaviour, one set of tests, on every era the client supports.</para>
