...
`Umpk.Data.Lang` supplies the vanilla `en_us` translation table for each of UMPK's 49 supported protocols. It depends only on [Umpk.Text](/packages/umpk-text). You can use it without the network, game model, or protocol packages.
...
DataGen emits the packed translation pool and protocol indexes from `data/java/<protocol>/lang.json`. Do not edit the two `.g.cs` files by hand. Use the three-output generation command in [the dataset](/concepts/the-dataset) and confirm that Git reports no generated diff.
