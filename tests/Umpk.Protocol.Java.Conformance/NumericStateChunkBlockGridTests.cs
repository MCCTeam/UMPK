...
/// <summary>The queryable-block-grid verification for the pre-flattening range (protocols 107-404). For every level_chunk frame in the pre-1.14 corpora, the production codec must decode the bit-packed paletted sections into a populated <see cref="ChunkColumn"/> (block states queryable), ALONGSIDE the verbatim wire body that the structural/relay path re-encodes. This proves the gameplay column is no longer a bare shell on 1.9-1.13.2: a spawn-area chunk decodes to at least one non-air block, and the decode still round-trips (asserted by the neighbouring structural/corpus conformance legs).</summary>
public sealed class NumericStateChunkBlockGridTests
{
    private static readonly Identifier LevelChunkId = PlayPackets.Clientbound.LevelChunk.Id;

    private readonly ITestOutputHelper _output;

    public NumericStateChunkBlockGridTests(ITestOutputHelper output) => _output = output;

    public static IEnumerable<object[]> Captures()
    {
        foreach (string path in CorpusLoader.DiscoverCaptures(FixturePaths.CorpusRoot))
            if (ProtocolFromPath(path) is >= 107 and <= 404)
                yield return [path];
    ...
    public async Task NumericStateChunkFrames_PopulateQueryableBlockGrid(string capturePath)
    {
        ...
        if (chunkFrames > 0)
            Assert.True(
                ...
                "the pre-1.14 section decode did not populate any block state (F5 regressed).");
